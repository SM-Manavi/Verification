...
typedef boost::unordered_map<std::string, std::vector<Robot*>> Neighbor_Matrix;
...
public:
	...
	void preComputation ()
    {
		std::vector<Robot*> neighbor_inf;
		std::vector<Robot>* robots = environment->getRobots();
        for(auto it1 = robots->begin() ; it1 != robots->end() ; ++it1)
        {
            for(auto it2 = robots->begin() ; it2 != robots->end() ; ++it2)
            {
                if( *it1  == *it2 )
                    continue;

                if(it1->communication_check(*it2))
                    neighbor_inf.push_back(&*it2);
            }

            neighbor_mat[it1->get_Name()] = neighbor_inf;
            neighbor_inf.clear();
        }
    }
